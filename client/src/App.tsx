...
function App() {
  ...
  const { currentPage, navigateToPage, showConfirmLeave, confirmLeave, cancelLeave } = useAppRouter();
    ...
    <ErrorBoundary>
      <ThemeProvider defaultTheme="light" switchable={true}>
        <TooltipProvider>
          ...
          <AlertDialog open={showConfirmLeave} onOpenChange={(open) => { if (!open) cancelLeave(); }}>
            <AlertDialogContent className="rounded-2xl">
              <AlertDialogHeader>
                <AlertDialogTitle className="font-display text-primary">มีข้อมูลที่กำลังกรอกค้างอยู่</AlertDialogTitle>
                <AlertDialogDescription>
                  คุณมีข้อมูลที่กำลังกรอกค้างอยู่ ต้องการออกจากหน้านี้หรือไม่? (ข้อมูลร่างของคุณจะยังคงอยู่)
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel onClick={cancelLeave} className="rounded-xl">ยกเลิก</AlertDialogCancel>
                <AlertDialogAction onClick={confirmLeave} className="rounded-xl">ต้องการออก</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </TooltipProvider>
